#!/usr/bin/env python3
"""
Core389 Master App v2.1 — full CORE025-style master lab + production-candidate workflow.
...
BUILD_MARKER = "BUILD: CORE389_MASTER_APP_V2_1_2026-06-08_UNIVERSAL_FAMILY_SCORE_APPLES_TO_APPLES"
FAMILY = "Core389"
MEMBERS = ["3389", "3889", "3899"]
DEFAULT_AUDIT_INTERVAL_DAYS = 90
DEFAULT_NEW_WIN_THRESHOLD = 25

st.set_page_config(page_title="Core389 Master App v2.1", layout="wide")
st.title("Core389 Master Lab v2.1 — Universal Family Score + Production Candidate")
...
def add_universal_family_score_389(playlist: pd.DataFrame, w_model: float = 0.45, w_row: float = 0.25, w_density: float = 0.20, w_outlier: float = 0.10) -> pd.DataFrame:
    """
    v2.1 Core025-parity universal family score for apples-to-apples cross-core comparison.
...
# -----------------------------
# v2.1 CORE389 ROW INTELLIGENCE LAB
...
st.sidebar.header("Master Validation")
run_full_validation=st.sidebar.checkbox("Run FULL CORE389 validation package", value=False, help="Runs true blind WF, row intelligence, budget cuts, migration, suppressed-member audit, and builds one validation ZIP. Slower but required before production decisions.")
st.sidebar.caption("Standard locked inputs: cleaned BASE4 history + validated Truth389 + validated Separator389. Optional overlays/reduction/rescue remain OFF unless you explicitly enable them.")
...
run_true_wf=st.sidebar.checkbox("Run TRUE blind WF diagnostic", value=False) or bool(run_full_validation)
...
st.subheader("Core389 v2.1 Master Workflow Status")
required_ok = {
...
if not hist.empty and not sep.empty:
    ...
    with st.expander("Core389 v2.1 TRUE Blind Walk-Forward / Master Row Intelligence Lab", expanded=True):
        st.caption("This is the real diagnostic path. It uses Truth389 chronologically: 60-row warmup by default, then only prior truth rows may calibrate rule weights. The uploaded separator is used as a candidate-rule library, not as future-outcome support.")
        if not run_true_wf:
            st.info("TRUE blind WF is available but not running. Turn on 'Run TRUE blind WF diagnostic' in the sidebar when you want the slower validation pass.")
        else:
            with st.spinner("Running true blind WF. This may take a minute because daily stream ranks are rebuilt from prior history only."):
            ...
            if true_wf.empty:
                ...
                st.dataframe(true_wf_summary, use_container_width=True, hide_index=True)
                ...
                st.dataframe(true_wf, use_container_width=True, hide_index=True, height=500)
                st.download_button("Download Core389 v2.1 TRUE Blind WF CSV", true_wf.to_csv(index=False).encode("utf-8"), file_name="core389_v21_true_blind_walkforward.csv", mime="text/csv")
                st.download_button("Download Core389 v2.1 TRUE Blind WF TXT", true_wf.to_csv(index=False, sep="\t").encode("utf-8"), file_name="core389_v21_true_blind_walkforward.txt", mime="text/plain")
                st.download_button("Download Core389 v2.1 TRUE Blind WF Summary CSV", true_wf_summary.to_csv(index=False).encode("utf-8"), file_name="core389_v21_true_blind_walkforward_summary.csv", mime="text/csv")
                st.download_button("Download Core389 v2.1 TRUE Blind WF Summary TXT", true_wf_summary.to_csv(index=False, sep="\t").encode("utf-8"), file_name="core389_v21_true_blind_walkforward_summary.txt", mime="text/plain")
                st.write("**Core389 v2.1 Row Intelligence Lab — Core025 method, Core389 data**")
                row_lab = build_core389_row_intelligence_lab_v21(true_wf, max_rank=max(streams_exported, 78))
                lab_tabs = st.tabs(["Budget cuts", "Row performance", "Rank migration", "Member x row", "3889 suppression"])
                with lab_tabs[0]:
                    st.caption("Tests how many blind WF winners would be inside a final printable cutoff. Full audit table remains uncut.")
                    st.dataframe(row_lab["budget_cutoffs"], use_container_width=True, hide_index=True)
                    st.download_button("Download Core389 v2.1 Budget Cut Lab CSV", row_lab["budget_cutoffs"].to_csv(index=False).encode("utf-8"), file_name="core389_v21_budget_cut_lab.csv", mime="text/csv")
                    st.download_button("Download Core389 v2.1 Budget Cut Lab TXT", row_lab["budget_cutoffs"].to_csv(index=False, sep="\t").encode("utf-8"), file_name="core389_v21_budget_cut_lab.txt", mime="text/plain")
                with lab_tabs[1]:
                    st.caption("Winning-row distribution by separate row systems: DynamicPlaylistRank, RawFitRank, SingleRow, StreamRank.")
                    st.dataframe(row_lab["row_profile"], use_container_width=True, hide_index=True, height=450)
                    st.download_button("Download Core389 v2.1 Row Performance Lab CSV", row_lab["row_profile"].to_csv(index=False).encode("utf-8"), file_name="core389_v21_row_performance_lab.csv", mime="text/csv")
                    st.download_button("Download Core389 v2.1 Row Performance Lab TXT", row_lab["row_profile"].to_csv(index=False, sep="\t").encode("utf-8"), file_name="core389_v21_row_performance_lab.txt", mime="text/plain")
                with lab_tabs[2]:
                    st.caption("How winning streams moved from raw fit rank to final dynamic playlist rank.")
                    st.dataframe(row_lab["movement_profile"], use_container_width=True, hide_index=True)
                    st.download_button("Download Core389 v2.1 Rank Migration Lab CSV", row_lab["movement_profile"].to_csv(index=False).encode("utf-8"), file_name="core389_v21_rank_migration_lab.csv", mime="text/csv")
                    st.download_button("Download Core389 v2.1 Rank Migration Lab TXT", row_lab["movement_profile"].to_csv(index=False, sep="\t").encode("utf-8"), file_name="core389_v21_rank_migration_lab.txt", mime="text/plain")
                with lab_tabs[3]:
                    st.caption("Truth-member distribution by winning dynamic PlaylistRank bucket.")
                    st.dataframe(row_lab["member_row_matrix"], use_container_width=True, hide_index=True)
                    st.download_button("Download Core389 v2.1 Member Row Matrix CSV", row_lab["member_row_matrix"].to_csv(index=False).encode("utf-8"), file_name="core389_v21_member_row_matrix.csv", mime="text/csv")
                    st.download_button("Download Core389 v2.1 Member Row Matrix TXT", row_lab["member_row_matrix"].to_csv(index=False, sep="\t").encode("utf-8"), file_name="core389_v21_member_row_matrix.txt", mime="text/plain")
                with lab_tabs[4]:
                    st.caption("Audit target for post-rank 3889 rescue: truth was 3889, but 3889 was Top2/Top3 instead of Top1.")
                    st.dataframe(row_lab["suppressed_3889"], use_container_width=True, hide_index=True, height=450)
                    st.download_button("Download Core389 v2.1 Suppressed 3889 Audit CSV", row_lab["suppressed_3889"].to_csv(index=False).encode("utf-8"), file_name="core389_v21_suppressed_3889_audit.csv", mime="text/csv")
                    st.download_button("Download Core389 v2.1 Suppressed 3889 Audit TXT", row_lab["suppressed_3889"].to_csv(index=False, sep="\t").encode("utf-8"), file_name="core389_v21_suppressed_3889_audit.txt", mime="text/plain")

                st.write("**Core389 after-stream ZLT audit candidates — audit only, OFF**")
                if zlt_audit.empty:
                    st.info("No ZLT audit candidates found at the current support threshold.")
                else:
                    st.warning("ZLT candidates are not enabled. They are after-stream/after-ranking trim candidates and must be validated on future/new rows before use.")
                    st.dataframe(zlt_audit, use_container_width=True, hide_index=True, height=350)
                    st.download_button("Download Core389 v1.5 ZLT Audit CSV", zlt_audit.to_csv(index=False).encode("utf-8"), file_name="core389_v15_zlt_audit_candidates.csv", mime="text/csv")
                    st.download_button("Download Core389 v1.5 ZLT Audit TXT", zlt_audit.to_csv(index=False, sep="\t").encode("utf-8"), file_name="core389_v15_zlt_audit_candidates.txt", mime="text/plain")

                st.success("Full validation run completed. The final ZIP package is available below after the printable playlist is built.")
    ...
    st.download_button("Download Core389 v2.1 TRUE DYNAMIC Full Audit Playlist CSV", csv, file_name="core389_v21_true_dynamic_full_audit_playlist.csv", mime="text/csv")
    st.download_button("Download Core389 v2.1 TRUE DYNAMIC Full Audit Playlist TXT", txt, file_name="core389_v21_true_dynamic_full_audit_playlist.txt", mime="text/plain")
    ...
    st.download_button("Download Printable Core389 Lab Playlist CSV", printable.to_csv(index=False).encode("utf-8"), file_name="core389_v21_printable_cleaned_play_only_playlist.csv", mime="text/csv")
    st.download_button("Download Printable Core389 Lab Playlist TXT", printable.to_csv(index=False, sep="\t").encode("utf-8"), file_name="core389_v21_printable_cleaned_play_only_playlist.txt", mime="text/plain")

    st.subheader("CORE389 v2.1 Validation Package")
    final_frames = {
        "core389_v21_true_dynamic_full_audit_playlist.csv": playlist,
        "core389_v21_printable_cleaned_play_only_playlist.csv": printable,
        "core389_v21_stream_cadence.csv": stream_cadence,
        "core389_v21_unique_streamkey_audit.csv": stream_audit,
        "core389_v21_current_day_rank_lookup.csv": backtest_diag,
    }
    if 'true_wf' in locals() and isinstance(true_wf, pd.DataFrame) and not true_wf.empty:
        final_frames.update({
            "core389_v21_true_blind_walkforward.csv": true_wf,
            "core389_v21_true_blind_walkforward_summary.csv": true_wf_summary,
            "core389_v21_budget_cut_lab.csv": row_lab.get("budget_cutoffs", pd.DataFrame()),
            "core389_v21_row_performance_lab.csv": row_lab.get("row_profile", pd.DataFrame()),
            "core389_v21_rank_migration_lab.csv": row_lab.get("movement_profile", pd.DataFrame()),
            "core389_v21_member_row_matrix.csv": row_lab.get("member_row_matrix", pd.DataFrame()),
            "core389_v21_suppressed_3889_audit.csv": row_lab.get("suppressed_3889", pd.DataFrame()),
            "core389_v21_zlt_audit_candidates.csv": zlt_audit,
        })
    manifest = "\n".join([
        BUILD_MARKER,
        "CORE389 v2.1 Master Lab + Production Candidate Validation Package",
    ...
    package_bytes = make_validation_zip_389(final_frames, {"core389_v21_validation_manifest.txt": manifest})
    st.download_button("Download CORE389 v2.1 Validation Package ZIP", package_bytes, file_name="core389_v21_validation_package.zip", mime="application/zip", use_container_width=True)

st.divider()
st.subheader("Core389 Notes")
st.markdown("""
- Core389 v2.1 status: **master lab / production-candidate only after full validation review**.
