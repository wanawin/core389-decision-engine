#!/usr/bin/env python3
"""
Core389 Lab App v1.5 — true blind walk-forward + ZLT audit, built from Core025 v182 workflow philosophy.
...
BUILD_MARKER = "BUILD: CORE389_LAB_APP_V1_5_2026-06-07_TRUE_BLIND_WALKFORWARD_ZLT_AUDIT"
FAMILY = "Core389"
...
st.warning("LAB ONLY: Core389 is not production-ready. Overlay/reduction/rescue parity scaffolds are OFF by default. True blind WF and ZLT audit are diagnostic only. Truth/Separator support is upload-only; generated files do not self-integrate.")
...
run_true_wf=st.sidebar.checkbox("Run TRUE blind WF diagnostic", value=False)
...
if not hist.empty and not sep.empty:
    ...
    with st.expander("Core389 v1.5 TRUE Blind Walk-Forward / Rank Diagnostic", expanded=True):
        ...
        if not run_true_wf:
            ...
            if true_wf.empty:
                ...
                st.download_button("Download Core389 v1.5 TRUE Blind WF CSV", true_wf.to_csv(index=False).encode("utf-8"), file_name="core389_v15_true_blind_walkforward.csv", mime="text/csv")
                st.download_button("Download Core389 v1.5 TRUE Blind WF TXT", true_wf.to_csv(index=False, sep="\t").encode("utf-8"), file_name="core389_v15_true_blind_walkforward.txt", mime="text/plain")
                st.download_button("Download Core389 v1.5 TRUE Blind WF Summary CSV", true_wf_summary.to_csv(index=False).encode("utf-8"), file_name="core389_v15_true_blind_walkforward_summary.csv", mime="text/csv")
                st.download_button("Download Core389 v1.5 TRUE Blind WF Summary TXT", true_wf_summary.to_csv(index=False, sep="\t").encode("utf-8"), file_name="core389_v15_true_blind_walkforward_summary.txt", mime="text/plain")
    ...
    st.download_button("Download Core389 v1.5 TRUE DYNAMIC Daily Lab Playlist CSV", csv, file_name="core389_v15_true_dynamic_daily_lab_playlist.csv", mime="text/csv")
    st.download_button("Download Core389 v1.5 TRUE DYNAMIC Daily Lab Playlist TXT", txt, file_name="core389_v15_true_dynamic_daily_lab_playlist.txt", mime="text/plain")
    ...
    st.download_button("Download Printable Core389 Lab Playlist CSV", printable.to_csv(index=False).encode("utf-8"), file_name="core389_v15_printable_true_dynamic_lab_playlist.csv", mime="text/csv")
    st.download_button("Download Printable Core389 Lab Playlist TXT", printable.to_csv(index=False, sep="\t").encode("utf-8"), file_name="core389_v15_printable_true_dynamic_lab_playlist.txt", mime="text/plain")
