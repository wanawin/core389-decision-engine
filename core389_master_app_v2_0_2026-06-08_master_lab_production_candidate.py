#!/usr/bin/env python3
"""
Core389 Master App v2.0 — full CORE025-style master lab + production-candidate workflow.
...
BUILD_MARKER = "BUILD: CORE389_MASTER_APP_V2_0_2026-06-08_MASTER_LAB_PRODUCTION_CANDIDATE"
FAMILY = "Core389"
MEMBERS = ["3389", "3889", "3899"]
DEFAULT_AUDIT_INTERVAL_DAYS = 90
DEFAULT_NEW_WIN_THRESHOLD = 25

st.set_page_config(page_title="Core389 Master App v2.0", layout="wide")
st.title("Core389 Master Lab + Production Candidate — 3389 / 3889 / 3899")
...
def build_core389_row_intelligence_lab_v20(wf: pd.DataFrame, max_rank: int = 78) -> Dict[str, pd.DataFrame]:
    ...
    out["RawToDynamicDeltaBucket"] = out["RawToDynamicDelta"].map(_v20_bucket_delta)
    out["WinnerPlaylistRankBucket"] = out["WinnerPlaylistRankNum"].map(_v20_bucket_rank)
...
st.subheader("Core389 v2.0 Master Workflow Status")
required_ok = {
...
if not hist.empty and not sep.empty:
    ...
    with st.expander("Core389 v2.0 TRUE Blind Walk-Forward / Master Row Intelligence Lab", expanded=True):
        ...
        if not run_true_wf:
            ...
            if true_wf.empty:
                ...
                st.download_button("Download Core389 v2.0 TRUE Blind WF CSV", true_wf.to_csv(index=False).encode("utf-8"), file_name="core389_v20_true_blind_walkforward.csv", mime="text/csv")
                st.download_button("Download Core389 v2.0 TRUE Blind WF TXT", true_wf.to_csv(index=False, sep="\t").encode("utf-8"), file_name="core389_v20_true_blind_walkforward.txt", mime="text/plain")
                st.download_button("Download Core389 v2.0 TRUE Blind WF Summary CSV", true_wf_summary.to_csv(index=False).encode("utf-8"), file_name="core389_v20_true_blind_walkforward_summary.csv", mime="text/csv")
                st.download_button("Download Core389 v2.0 TRUE Blind WF Summary TXT", true_wf_summary.to_csv(index=False, sep="\t").encode("utf-8"), file_name="core389_v20_true_blind_walkforward_summary.txt", mime="text/plain")
                st.write("**Core389 v2.0 Row Intelligence Lab — Core025 method, Core389 data**")
                row_lab = build_core389_row_intelligence_lab_v20(true_wf, max_rank=max(streams_exported, 78))
                ...
                with lab_tabs[0]:
                    ...
                    st.download_button("Download Core389 v2.0 Budget Cut Lab CSV", row_lab["budget_cutoffs"].to_csv(index=False).encode("utf-8"), file_name="core389_v20_budget_cut_lab.csv", mime="text/csv")
                    st.download_button("Download Core389 v2.0 Budget Cut Lab TXT", row_lab["budget_cutoffs"].to_csv(index=False, sep="\t").encode("utf-8"), file_name="core389_v20_budget_cut_lab.txt", mime="text/plain")
                with lab_tabs[1]:
                    ...
                    st.download_button("Download Core389 v2.0 Row Performance Lab CSV", row_lab["row_profile"].to_csv(index=False).encode("utf-8"), file_name="core389_v20_row_performance_lab.csv", mime="text/csv")
                    st.download_button("Download Core389 v2.0 Row Performance Lab TXT", row_lab["row_profile"].to_csv(index=False, sep="\t").encode("utf-8"), file_name="core389_v20_row_performance_lab.txt", mime="text/plain")
                with lab_tabs[2]:
                    ...
                    st.download_button("Download Core389 v2.0 Rank Migration Lab CSV", row_lab["movement_profile"].to_csv(index=False).encode("utf-8"), file_name="core389_v20_rank_migration_lab.csv", mime="text/csv")
                    st.download_button("Download Core389 v2.0 Rank Migration Lab TXT", row_lab["movement_profile"].to_csv(index=False, sep="\t").encode("utf-8"), file_name="core389_v20_rank_migration_lab.txt", mime="text/plain")
                with lab_tabs[3]:
                    ...
                    st.download_button("Download Core389 v2.0 Member Row Matrix CSV", row_lab["member_row_matrix"].to_csv(index=False).encode("utf-8"), file_name="core389_v20_member_row_matrix.csv", mime="text/csv")
                    st.download_button("Download Core389 v2.0 Member Row Matrix TXT", row_lab["member_row_matrix"].to_csv(index=False, sep="\t").encode("utf-8"), file_name="core389_v20_member_row_matrix.txt", mime="text/plain")
                with lab_tabs[4]:
                    ...
                    st.download_button("Download Core389 v2.0 Suppressed 3889 Audit CSV", row_lab["suppressed_3889"].to_csv(index=False).encode("utf-8"), file_name="core389_v20_suppressed_3889_audit.csv", mime="text/csv")
                    st.download_button("Download Core389 v2.0 Suppressed 3889 Audit TXT", row_lab["suppressed_3889"].to_csv(index=False, sep="\t").encode("utf-8"), file_name="core389_v20_suppressed_3889_audit.txt", mime="text/plain")
    ...
    if use_overlay:
        ...
        st.info("Experimental overlay is OFF. v1.5 true dynamic PlaylistRank is shown/exported; true blind WF and ZLT audit are diagnostics only; raw fit rank remains visible as RawFitRank/Core389FitRank.")
    ...
    st.download_button("Download Core389 v2.0 TRUE DYNAMIC Full Audit Playlist CSV", csv, file_name="core389_v20_true_dynamic_full_audit_playlist.csv", mime="text/csv")
    st.download_button("Download Core389 v2.0 TRUE DYNAMIC Full Audit Playlist TXT", txt, file_name="core389_v20_true_dynamic_full_audit_playlist.txt", mime="text/plain")
    ...
    st.download_button("Download Printable Core389 Lab Playlist CSV", printable.to_csv(index=False).encode("utf-8"), file_name="core389_v20_printable_cleaned_play_only_playlist.csv", mime="text/csv")
    st.download_button("Download Printable Core389 Lab Playlist TXT", printable.to_csv(index=False, sep="\t").encode("utf-8"), file_name="core389_v20_printable_cleaned_play_only_playlist.txt", mime="text/plain")

    st.subheader("CORE389 v2.0 Validation Package")
    final_frames = {
        "core389_v20_true_dynamic_full_audit_playlist.csv": playlist,
        "core389_v20_printable_cleaned_play_only_playlist.csv": printable,
        "core389_v20_stream_cadence.csv": stream_cadence,
        "core389_v20_unique_streamkey_audit.csv": stream_audit,
        "core389_v20_current_day_rank_lookup.csv": backtest_diag,
    }
    if 'true_wf' in locals() and isinstance(true_wf, pd.DataFrame) and not true_wf.empty:
        final_frames.update({
            "core389_v20_true_blind_walkforward.csv": true_wf,
            "core389_v20_true_blind_walkforward_summary.csv": true_wf_summary,
            "core389_v20_budget_cut_lab.csv": row_lab.get("budget_cutoffs", pd.DataFrame()),
            "core389_v20_row_performance_lab.csv": row_lab.get("row_profile", pd.DataFrame()),
            "core389_v20_rank_migration_lab.csv": row_lab.get("movement_profile", pd.DataFrame()),
            "core389_v20_member_row_matrix.csv": row_lab.get("member_row_matrix", pd.DataFrame()),
            "core389_v20_suppressed_3889_audit.csv": row_lab.get("suppressed_3889", pd.DataFrame()),
            "core389_v20_zlt_audit_candidates.csv": zlt_audit,
        })
    manifest = "\n".join([
        BUILD_MARKER,
        "CORE389 v2.0 Master Lab + Production Candidate Validation Package",
    ...
    package_bytes = make_validation_zip_389(final_frames, {"core389_v20_validation_manifest.txt": manifest})
    st.download_button("Download CORE389 v2.0 Validation Package ZIP", package_bytes, file_name="core389_v20_validation_package.zip", mime="application/zip", use_container_width=True)

st.divider()
st.subheader("Core389 Notes")
st.markdown("""
- Core389 v2.0 status: **master lab / production-candidate only after full validation review**.
